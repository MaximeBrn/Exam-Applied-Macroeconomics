...
% Variables in Figure 1.a
yirf =  cumsum(E_GY_E_EPS_IG); % Output growth
cirf =  cumsum(E_GC_E_EPS_IG); % Private consumption growth
iirf =  cumsum(E_GI_E_EPS_IG); % Private investment growth
cricirf =  cumsum(E_GCNLC_E_EPS_IG); % Private consumption growth (ricardian HH)
cliqcirf =  cumsum(E_GCLC_E_EPS_IG); % Private consumption growth (non-Ricardian HH)
tbynirf =  E_TBYN_E_EPS_IG; % Nominal trade balance to GDP share
ggirf =  cumsum(E_GG_E_EPS_IG);  % Gov consumption growth
gigirf =  cumsum(E_GIG_E_EPS_IG); % Gov investment growth
gtrirf =  cumsum(E_GTR_E_EPS_IG); % Transfers growth

% Variables in Figure 1.b
glirf =  cumsum(E_GL_E_EPS_IG); % Employment rate growth
rwirf =  cumsum(E_GWRY_E_EPS_IG+E_GY_E_EPS_IG); % (real wages/real gdp) growth 
dbgynirf =  E_DBGYN_E_EPS_IG; % Deficit
ygapirf =  E_LYGAP_E_EPS_IG; % Log ouput gap
phiirf = E_PHI_E_EPS_IG % Inflation
infirf = E_PHIC_E_EPS_IG; % Inflation of C deflator
inomirf = E_INOM_E_EPS_IG; % Nominal interest rate
rirf =  E_R_E_EPS_IG; % Real interest rate
geirf =  cumsum(E_GE_E_EPS_IG); % Nominal exchange rate

fiscalmultiplier=yirf./gigirf/IGSN % Formula of investment fiscal multiplier
...
plot(t,fiscalmultiplier,'LineWidth',2);
xlabel('quarters','FontSize',8);
title('Government investment multipliers','FontSize',10);
